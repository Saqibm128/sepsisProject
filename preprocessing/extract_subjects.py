...
parser.add_argument('--use_db', action='store_true', help='Use a database instead of CSV files', default=False)
#TODO: fix this part to work? or use separate args for all config
...
if (args.use_db):
    try:
        globalConfig = json.load(open("config.json", "r"))
    except:
...
stays = read_icustays_table(args.mimic3_path, use_db=args.use_db, conn_info=globalConfig)
if args.verbose:
...
diagnoses = read_icd_diagnoses_table(args.mimic3_path, use_db=args.use_db, conn_info=globalConfig)
