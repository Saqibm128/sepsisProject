...
-- Aggregate
  ...
  distinctAdmissions as (
    ...
    SELECT DISTINCT hadm_id, specificlabEvents.itemid
    FROM specificlabEvents
  )
    -- If we take multiple tests each hospital admission, we only count it once
    SELECT distinctAdmissions.itemid, d_labitems.label, COUNT(distinctAdmissions.itemid) as specificLabEventsCountsDistinctAdmissions
    FROM distinctAdmissions
		JOIN d_labitems on d_labitems.itemid = distinctAdmissions.itemid
    GROUP BY d_labitems.itemid, distinctAdmissions.itemid, d_labitems.label
		ORDER BY specificLabEventsCountsDistinctAdmissions
